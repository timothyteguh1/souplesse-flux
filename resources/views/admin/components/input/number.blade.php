...
@if ($defer)
    <input inputmode="decimal" x-data="{ value: @entangle($name) }" x-init="() => {
        ...
        x-on:blur="value = $el.__cleave.getRawValue()" x-effect="if($el.__cleave) $el.__cleave.setRawValue(value)"
        {{ $attributes->merge(['class' => 'form-control' . ($errors->has($name) ? ' is-invalid' : '')]) }} @if ($disabled)
    disabled
@endif/>
@else
