...
use App\Models\Persediaan\RekalkulasiMutasiStok;
...
class MutasiStokService
{
    ...
    public static function decrease(
        ...
        //        $saldoProdukTransaksi = InventoryFunction::getStok($produk_id, $cabang_id, $gudang_id, $tanggal, expired_date: $expired_date, no_batch: $no_batch);
        //
        ...
        //        $saldoProduk = InventoryFunction::getStok($produk_id, $cabang_id, $gudang_id, expired_date: $expired_date, no_batch: $no_batch);
        //
        ...
        foreach ($dataStokByExpiredDates as $dataStokByExpiredDate) {
            ...
            foreach ($dataStokByNoBatchs as $dataStokByNoBatch) {
                ...
                if ($jumlah + $dataStokByNoBatch->total_jumlah < 0) {
                    if ($dataStokByNoBatch->expired_date) {
                ...
                // saldo produk pada saat transaksi
                // $saldoProdukTransaksi = InventoryFunction::getStok($produk_id, $cabang_id, $gudang_id, $tanggal);
                // if ($saldoProdukTransaksi + $jumlah < 0) {
                //     throw new GeneralException("Qty baru dari {$produk->nama} pada saat transaksi tidak boleh kurang dari 0.");
                // }

                // saldo produk aktual, dalam satuan terkecil
                $saldoProduk = InventoryFunction::getStok($produk_id, $cabang_id, $gudang_id);
    ...
    public static function destroy(?MutasiStok $mutasiStok = null): bool
    {
        ...
        //        $saldoProdukTransaksi = InventoryFunction::getStok($produk->id, $mutasiStok->cabang_id, $mutasiStok->gudang_id, _datetime_carbon_db($mutasiStok->tanggal), expired_date: _date_format_db($mutasiStok->expired_date), no_batch: $mutasiStok->no_batch);
        //
        ...
        //
        //        // saldo produk aktual, dalam satuan terkecil
        //        $saldoProduk = InventoryFunction::getStok($produk->id, $mutasiStok->cabang_id, $mutasiStok->gudang_id, expired_date: _date_format_db($mutasiStok->expired_date), no_batch: $mutasiStok->no_batch);
    ...
    // public static function recalculation(RekalkulasiMutasiStok $obj): array
    // {
    //     $results = [];
    //     $tanggal_awal = _datetime_carbon_db($obj->tanggal_awal)->startOfDay();
    //     $tanggal_akhir = _datetime_carbon_db($obj->tanggal_akhir)->endOfDay();
    //     $mutasiStoks = MutasiStok::query()
    //         ->with(['reference', 'header'])
    //         ->when($obj->produk_id, function ($query) use ($obj) {
    //             return $query->where('produk_id', $obj->produk_id);
    //         })
    ...
    //         $harga = InventoryFunction::getHpp($mutasiStok->cabang_id, $mutasiStok->produk_id, $tanggal_akhir);
    //         if ($harga != $mutasiStok->harga) {
    //             $results[] = clone $mutasiStok;
    //             $mutasiStok->update([
    //                 'harga' => $harga,
