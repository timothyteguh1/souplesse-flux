<?php

namespace App\Livewire\Admin\Master\KategoriProduk;

use App\Imports\KategoriProdukImport;
use App\Models\Master\KategoriProduk;
use App\Traits\Livewire\WithImportForm;
use Livewire\Component;

class Import extends Component
{
    use WithImportForm;

    public $model = KategoriProduk::class;
    public $menuTitle = 'Kategori Produk';
    protected $template_filename = 'template_kategori_produk';
    protected $template_view = 'admin.master.kategori-produk.import-template';
    public $cabang_id;
    public $file;

    protected function rules(): array
    {
        return [
            'cabang_id' => ['required'],
            'file' => ['required', 'file', 'mimes:xlsx,xls,csv'],
        ];
    }

    public function mount()
    {
        $this->checkPermissionCreateGate();
        $this->cabang_id = session()->get('cabang_id');
    }

    public function submit($validated)
    {
        $import = new KategoriProdukImport($this->cabang_id);
        $import->import($validated['file']);
        return $import->count;
    }

    public function render()
    {
        return view('admin.master.kategori-produk.import')->layout($this->layout);
    }
}
