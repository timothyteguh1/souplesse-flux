...
class Create extends Component
{
    ...
    public $cabang_id;
    public $kode;
    public $jenis_transaksi = Const_Umum::JENIS_TRANSAKSI_FAKTUR_PENJUALAN_KREDIT;
    public $tanggal;
    ...
    public function cekPromoGrosir2()
    {
        $arrSuppliers = [];
        foreach ($this->items as $item) {
            $produk = Produk::find($item['produk_id']);
            if (isset($arrSuppliers[$produk?->supplier_id])) {
                $arrSuppliers[$produk?->supplier_id]['jumlah'] += $item['jumlah'];
            } else {
                $arrSuppliers[$produk?->supplier_id] = [
                    'jumlah' => $item['jumlah'],
        ...
        $promos = Promo::where('is_promo_grosir', true)
            ...
            ->whereDate('tanggal_awal', '<=', Carbon::now())
            ->whereDate('tanggal_akhir', '>=', Carbon::now())
            ->with('promoSuppliers')->get();
        $eligiblePromos = [];

        foreach ($promos as $ii => $promo) {
            $applies = false;

            foreach ($arrSuppliers as $supplierId => $data) {
                $subtotal = $data['subtotal'];
                $jumlah = $data['jumlah'];
                // Jika promo tidak punya suppliers → berlaku semua
                if ($promo->promoSuppliers->isEmpty() || $promo->promoSuppliers->contains('id', $supplierId)) {
                    // Cek syarat minimal
                    if (
                        ($promo->min_pembelian_rp == 0 || $subtotal >= $promo->min_pembelian_rp) ||
                        ($promo->min_pembelian_jumlah == 0 || $jumlah >= $promo->min_pembelian_jumlah)
                    ) {
                        $applies = true;
                    }
                }
        ...
        // pilih diskon paling besar
        $bestPromo = collect($eligiblePromos)->sortByDesc('diskon_satuan_1')->first();
        if (!$bestPromo) {
            foreach ($this->items as $index => $item) {
                if ($this->items[$index]['is_promo_grosir_applied']) {
            ...
            foreach ($this->items as $index => $item) {
                if (!$this->items[$index]['is_promo_grosir_applied']) {
                    ...
                    $this->items[$index]['diskon_satuan_type_4'] = $diskon_satuan_type_3;
                    $this->items[$index]['is_promo_grosir_applied'] = true;
                } elseif ($this->items[$index]['is_promo_grosir_applied'] && $bestPromo['diskon_satuan_1'] != $this->items[$index]['diskon_satuan_1']) {
                    $this->items[$index]['diskon_satuan_1'] = $bestPromo['diskon_satuan_1'];
                    $this->items[$index]['diskon_satuan_type_1'] = $bestPromo['diskon_satuan_type_1'];
                }
            }
    ...
    public function edit($index)
    {
        ...
        $this->input_subtotal = $item['jumlah'] * $item['harga_satuan'];
    ...
    public function render()
    {
        ...
        return view('admin.penjualan.faktur-penjualan.create')->layout($this->layout);
    }
}
