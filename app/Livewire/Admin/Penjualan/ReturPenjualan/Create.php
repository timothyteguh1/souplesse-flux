...
class Create extends Component
{
    ...
    public function editItem()
    {
        ...
        $ppn = 0;
        $subtotal = 0;
        $harga_net = $harga_satuan - $diskon_satuan;
        if ($this->is_include_ppn) {
            $ppn = _ppn_value($harga_net, $this->ppn_percent, true) * $jumlah;
            $subtotal = ($harga_net * $jumlah);
        } else {
            $ppn = _ppn_value($harga_net, $this->ppn_percent) * $jumlah;
            $subtotal = ($harga_net * $jumlah) + $ppn;
        }
    ...
    {
        return ReturPenjualanService::create($validated);
    }
    ...
    public function render()
    {
        ...
        return view('admin.penjualan.retur-penjualan.create')->layout($this->layout);
    }
}
